...
class AudioVisualizer:
        ...
        try:
            ...
            if hasattr(audio, 'chapters'):
                ...
                for chapter in audio.chapters:
                    start = chapter.start_time / 1000.0
    ...
    async def export_video(self):
        ...
        video_stream = ffmpeg.input('pipe:0', format='rawvideo', pix_fmt='rgb24', s=f'{width}x{height}', r=30)
        audio_stream = ffmpeg.input(self.audio_path)

        process = (
            ffmpeg
            .output(video_stream, audio_stream, out_path, pix_fmt='yuv420p', vcodec='libx264', acodec='aac', r=30)
            .overwrite_output()
    ...
    async def generate_video_with_svd(self):
        if not self.image_path or not self.svd_pipeline:
            self.status_label.text = "❌ Image or SVD model not loaded"
            return

        self.status_label.text = "⏳ Generating video with SVD..."
        await asyncio.sleep(0.1)
        ...
        try:
            ...
            frames = self.svd_pipeline(image, decode_chunk_size=8, num_frames=25).frames[0]
            video_path = export_to_video(frames, "generated_video.mp4", fps=8)

            # Overlay waveform and audio
            waveform_video = await self.create_waveform_video()
            final_video = self.combine_videos(video_path, waveform_video, self.audio_path)
            self.last_export_path = final_video
            self.status_label.text = f"✓ Video generated: {final_video}"
        except Exception as e:
            self.status_label.text = f"❌ Video generation failed: {e}"

    async def create_waveform_video(self):
        # Use FFmpeg to generate waveform video
        waveform_path = 'waveform.mp4'
        (
            ffmpeg
            .input(self.audio_path)
            .filter('showwaves', s='1280x720', colors='Cyan', mode='cline')
            .output(waveform_path, vcodec='libx264', acodec='aac', r=30)
            .overwrite_output()
            .run(cmd=self.ffmpeg_cmd)
        )
        return waveform_path

    def combine_videos(self, base_video, overlay_video, audio_path):
...
def create_ui():
    ...
    with ui.card().style('max-width: 1200px; margin: auto;'):
        ui.label('🎵 Audio Visualizer with Video Generation').style('font-size: 24px; font-weight: bold; color: #00d4ff; text-align: center;')
        ui.label('Upload audio and image to generate stylized videos with waveforms.').style('color: #94a3b8; text-align: center;')
        ...
        with ui.row():
            ui.upload(
                ...
                on_rejected=lambda e: visualizer.status_label.set_text('❌ Audio upload rejected')
            ).props('accept=audio/*')
            ui.upload(
                ...
                on_rejected=lambda e: visualizer.status_label.set_text('❌ Image upload rejected')
            ).props('accept=image/*')

        visualizer.loading_indicator = ui.spinner(size='lg')
        visualizer.loading_indicator.classes('hidden')

        visualizer.status_label = ui.label("Ready to upload").style('color: #4ade80; font-weight: 500;')
        ...
        ui.button('🎬 Generate Video with SVD', on_click=visualizer.generate_video_with_svd)

@ui.page('/visualizer')
def visualizer_page():
    if CURRENT_VISUALIZER is None or not (CURRENT_VISUALIZER.audio_ready and CURRENT_VISUALIZER.image_ready):
        ui.label('No data yet. Upload audio and image first.').style('color: #94a3b8;')
        return
...
if __name__ in ("__main__", "__mp_main__"):
    ui.run(port=8080, title='Audio Visualizer with Video Gen')