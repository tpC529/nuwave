...
# ── FFmpeg binary ──────────────────────────────────────────────────────────────
# On Linux servers (Render, Railway) prefer the system ffmpeg installed via
# apt/build command. Fall back to the imageio_ffmpeg bundled binary for Windows.
def _find_ffmpeg() -> str:
    # 1. Explicit override via env var (e.g. FFMPEG_BINARY=C:\ffmpeg\bin\ffmpeg.exe)
    env_path = os.environ.get('FFMPEG_BINARY', '')
    if env_path and os.path.isfile(env_path):
        return env_path
    # 2. System ffmpeg (Linux — installed via apt in Render build command)
    try:
        ...
        pass
    # 3. imageio_ffmpeg bundled binary (Windows local dev)
    try:
        import imageio_ffmpeg
        return imageio_ffmpeg.get_ffmpeg_exe()
    except Exception:
        pass
    return 'ffmpeg'   # last resort — let the OS find it

FFMPEG_CMD = _find_ffmpeg()
print(f'  FFmpeg: {FFMPEG_CMD}')
if PYDUB_OK:
    AudioSegment.converter = FFMPEG_CMD
    AudioSegment.ffmpeg    = FFMPEG_CMD
os.environ['PATH'] = os.path.dirname(os.path.abspath(FFMPEG_CMD)) + os.pathsep + os.environ.get('PATH', '')
...
def _detect_nvenc() -> bool:
    """
    Test for NVENC with a hard 5-second timeout so a missing/slow FFmpeg
    binary never blocks startup on cloud servers (Render, Railway, etc).
    """
    try:
        r = subprocess.run(
            ...
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            timeout=5,   # ← never block more than 5 s at startup
        )
        return r.returncode == 0
    except Exception:   # TimeoutExpired, FileNotFoundError, etc.
        return False

# Detect at startup but never block — timeout protects cloud deployments
...
VID_H    = 480
FPS      = 24
LTX_FRAMES = 97      # (97-1)%8==0 ✓  minimum valid LTX frame count
LTX_STEPS  = 25      # 25 = best speed/quality balance (~40% faster than 40, minimal quality loss)
...
def _load_ltx():
    ...
    try:
        ...
        # .to('cuda') keeps full model in VRAM — much faster than cpu_offload.
        # RTX 5070 8GB handles LTX-Video in bfloat16 without offloading.
        # If you ever get OOM errors, swap back to pipe.enable_model_cpu_offload()
        pipe = pipe.to('cuda')
        pipe.vae.enable_tiling()    # prevents OOM during VAE decode
        pipe.vae.enable_slicing()   # decodes latents in slices, saves VRAM
        # Enable memory-efficient attention if xformers is installed
        try:
            pipe.enable_xformers_memory_efficient_attention()
            print('  xformers attention enabled')
        except Exception:
            pass  # fine without it
        LTX_PIPELINE = pipe
...
def generate_video(audio_file, image_path, progress=None):
    ...
    if ltx_ok:
        _prog(0.12, f'LTX-Video: generating {LTX_FRAMES} frames (~2-4 min on RTX 5070)…')
        try:
...
if __name__ == '__main__':
    ...
    app.launch(
        ...
        quiet=False,        # keep logs visible in Render dashboard
        show_error=True,    # surface tracebacks in the UI
